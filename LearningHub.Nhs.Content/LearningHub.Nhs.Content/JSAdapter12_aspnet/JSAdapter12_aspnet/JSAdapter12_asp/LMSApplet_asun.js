<!-- $Header: LMSApplet_asun.js 120.0.12010000.3 2021/06/17 07:49:04 smahanka noship $ -->
...
function LMSApplet(lmsUrl,attemptId,respId,respApplId,secGroupId,debug)
{
   ...
   this._initialize(lmsUrl,attemptId,respId,respApplId,secGroupId,debug);
}

LMSApplet.prototype._initialize=function(lmsUrl,attemptId,respId,respApplId,secGroupId,debug)
{
   this._elements=null;
   ...
   this._lmsDebug=debug;
   this._isContentUnloading = false;
   // requestQueue facilitates a way to execute the requests in an asynchronous fashion, 
   // so all we have to do is to add all the necessary requests to the RequestQueue using the add method.
...
LMSApplet.prototype.commit=function()
{
   if(this._dirtyElements!=null)
   {
      ...
      try{
         // As latest browsers are not supporting the synchronous calls during the window unload phase, 
         // RequestQueue facilitates a way to execute them in an asynchronous fashion, 
         // so all we have to do is to add all the necessary requests to the RequestQueue using the add method 
         // and they would be completed by the Queue, if one call fails it doesn't execute rest of the calls and 
         // informs the user with an error alert.
         if(this._isContentUnloading){
            this._requestQueue.add(this._dirtyElements.get("action"), this.prepareSendRequestData(this._dirtyElements), this);
         }else{ 
...
LMSApplet.prototype.initialize=function()
{
   if(!this._initialized)
   {
      ...
      // informs the user with an error alert.
      if(this._isContentUnloading){
         this._requestQueue.add(h.get("action"), this.prepareSendRequestData(h), this);
      }else{ 
         this.sendRequest(h);
      }
...
LMSApplet.prototype.setValue=function(element,value)
{
   ...
   this.getElements().set(element,value.toString()); //CHG2006381 Convert value toString
   if(dirty)
   {
      try
      {
...
LMSApplet.prototype.sendRequest=function(h)
{
   ...
   var msg=this.prepareSendRequestData(h);
   this.log("Reply:",1);
   var responseString=this.sendSyncRequest(msg);
   var response=this.processResponseString(responseString);
...
/**
 * Notification that the player is unloading the content window. If the 
 * content is being unloaded, all requests to the LMS will be made asynchronously. 
 **/
LMSApplet.prototype.setContentUnloading=function(isUnloading){
   this._isContentUnloading = isUnloading;
};
 
/**
 * Converts data to string format.
 **/
LMSApplet.prototype.prepareSendRequestData=function(h)
{
   ...
   return msg.toEncodedString();
};

/**
 * Converting responseString to Hashtable.
 **/
LMSApplet.prototype.processResponseString=function(responseString)
{
   ...
   for(var i in lines)
   {
      ...
      {
         var regexp = /<!--.*-->/;
         var result = regexp.test(lines[i]);
         if(!result)
         {
            this.log(lines[i],2);
         }
...
LMSApplet.prototype.sendAsyncRequest=function(data)
{
   // As we can't use 'this' in anonymous functions, 'self' here helps us in referring to the context 
   var self = this;
   return new Promise((resolve, reject) => {
      var postRequest = new XMLHttpRequest();
      postRequest.open('POST', self._lmsUrl);
      postRequest.onreadystatechange = function () {
          if (postRequest.readyState == XMLHttpRequest.DONE) {
              if (postRequest.status == 200) {
                  resolve(postRequest.responseText);
              } else {
                  self.log("Asynchronous Data posting failed");
                  reject("Unable to post data with status - " + postRequest.status);
              }
          }
